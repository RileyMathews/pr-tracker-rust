...
mod tests {
    ...
    #[test]
    fn parse_github_timestamp_empty_is_unix_epoch() {
        let parsed = parse_github_timestamp("").expect("timestamp should parse");
        assert_eq!(parsed, DateTime::UNIX_EPOCH);
    }

    #[test]
    fn parse_github_timestamp_invalid_fails() {
        let result = parse_github_timestamp("not-a-timestamp");
        assert!(result.is_err());
    }

    #[test]
    fn parse_optional_timestamp_none_returns_none() {
        let parsed = parse_optional_timestamp(None);
        assert_eq!(parsed, None);
    }

    #[test]
    fn parse_optional_timestamp_invalid_returns_none() {
        let parsed = parse_optional_timestamp(Some("invalid"));
        assert_eq!(parsed, None);
    }
    ...
    fn filter_new_prs_mixed() {
        let prs = vec![
            ...
            discovery_pr(2, Some("bob")),
            discovery_pr(3, Some("alice")),
            discovery_pr(4, Some("carol")),
        ];
